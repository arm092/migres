...
from migres.logger import setup_logging
from migres.snapshot import run_snapshot, SnapshotError
from migres.producer import PipelineProducer
from migres.transformer import PipelineTransformer
from migres.consumer import PipelineConsumer
...
def _signal_terminate(signum, frame):
    logging.info("Received terminate signal (%s). Queuing graceful shutdown...", signum)
    _signal_flags["terminate"].set()
...
def _start_pipeline_threads(cfg):
    ...
    # Non-daemon so SIGTERM/KeyboardInterrupt can flush producer batches before exit
    producer_thread = threading.Thread(target=producer.run, name="Producer", daemon=False)
    transformer_thread = threading.Thread(target=transformer.run, name="Transformer", daemon=False)
    consumer_thread = threading.Thread(target=consumer.run, name="Consumer", daemon=False)
    ...
    try:
        while True:
            ...
            if _signal_flags["reposition"].is_set():
                ...
                _perform_reposition()
            if _signal_flags["terminate"].is_set():
                logging.info("Graceful shutdown: flushing pipeline stages...")
                _shutdown_pipeline_threads(timeout=60)
                notify_cdc_shutdown("Graceful shutdown (SIGTERM/SIGINT)")
                sys.exit(0)
        ...
        logging.info("Stopping CDC pipeline (KeyboardInterrupt)...")
        _signal_flags["terminate"].set()
        _shutdown_pipeline_threads(timeout=60)
        notify_cdc_shutdown("Interrupted")
        sys.exit(0)
...
def main():
    ...
    try:
        ...
        logging.info("Terminate signal handler registered (SIGTERM)")
    except (AttributeError, OSError) as e:
        logging.warning(f"Cannot register SIGTERM handler: {e}")
    try:
